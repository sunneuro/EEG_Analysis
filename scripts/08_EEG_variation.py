...
def main():
    ...
    print("=" * 70)
    print(f"{task.upper()} EEG VARIABILITY ANALYSIS")
    print(f"Started : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)

    # ── Load Data ─────────────────────────────────────────────────────────────────
    csv_path = group_folder / f'group_{task}_individual_components.csv'
    if not csv_path.exists():
        print(f"ERROR: {csv_path} not found. Run 07_group_erp.py first.")
        sys.exit(1)
    ...
    for cond in conditions:
        for metric in metrics:
            ...
            # Filter data
            g1_vals = df[(df['component'] == comp) & (df['condition'] == cond) & (df['group'] == g1)][mcol].dropna().values
            g2_vals = df[(df['component'] == comp) & (df['condition'] == cond) & (df['group'] == g2)][mcol].dropna().values
            
            # Descriptive statistics
            for grp, vals in [(g1, g1_vals), (g2, g2_vals)]:
                n = len(vals)
                if n > 0:
            ...
            if len(g1_vals) >= 2 and len(g2_vals) >= 2:
                levene_stat, levene_p = stats.levene(g1_vals, g2_vals, center='median')
                sd_ctrl = np.std(g1_vals, ddof=1)
                sd_creat = np.std(g2_vals, ddof=1)
                var_ratio = (sd_ctrl ** 2) / (sd_creat ** 2) if sd_creat > 0 else np.nan
            else:
                levene_stat, levene_p = np.nan, np.nan
                sd_ctrl = np.std(g1_vals, ddof=1) if len(g1_vals) > 1 else np.nan
                sd_creat = np.std(g2_vals, ddof=1) if len(g2_vals) > 1 else np.nan
                var_ratio = np.nan
            ...
            results.append({
                ...
                'component': comp,
                'measure': mname,
                f'sd_{g1}': round(sd_ctrl, 4) if not np.isnan(sd_ctrl) else np.nan,
                f'sd_{g2}': round(sd_creat, 4) if not np.isnan(sd_creat) else np.nan,
    ...
    descriptives_path = stats_folder / f'{task}_EEG_variability_descriptives.csv'
    statistics_path = stats_folder / f'{task}_EEG_variability_statistics.csv'
    ...
    for r in results:
        ...
        print(f"  {r['condition']:15s} {r['measure']:25s} "
              f"{fmt_val(r[f'sd_{g1}']):>8} {fmt_val(r[f'sd_{g2}']):>8} "
    ...
    for cond_code in conditions:
        ...
        for ax, metric in zip(axes, metrics):
            ...
            # Get data
            g1_vals = df[(df['component'] == comp) & (df['condition'] == cond_code) & (df['group'] == g1)][mcol].dropna().values
            g2_vals = df[(df['component'] == comp) & (df['condition'] == cond_code) & (df['group'] == g2)][mcol].dropna().values
            
            sd_ctrl = np.std(g1_vals, ddof=1) if len(g1_vals) > 1 else 0.0
            sd_creat = np.std(g2_vals, ddof=1) if len(g2_vals) > 1 else 0.0
            var_ratio = (sd_ctrl ** 2) / (sd_creat ** 2) if sd_creat > 0 else np.nan
            ...
            ax.set_xticks([0, 1])
            ax.set_xticklabels([group_labels[g] for g in [g1, g2]], fontsize=14)
            ax.set_xlim([-0.5, 1.5])
        ...
        # Save figure
        fname = f"group_{task}_variability_{file_suffixes[cond_code]}.png"
        plt.suptitle(
            f"{task.upper()} {cond_lbl.upper()} EEG VARIABILITY — {group_labels[g1]} vs {group_labels[g2]}",
            fontsize=15, fontweight='bold', y=0.99
        )
        fig.text(
            0.5, 0.95,
            f"Dotted/Light Grey = {group_labels[g1]} · Solid/Darker Grey = {group_labels[g2]}\n"

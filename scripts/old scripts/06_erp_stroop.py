# scripts/06_erp_stroop.py
# ─────────────────────────────────────────────────────────────────────────────
# STEP 4 OF EEG PIPELINE: ERP averaging and component extraction — STROOP
#
...
#   P3b  : Pz,      300–600 ms  — peak latency ONLY (same window as P300,
#                                  named separately for latency comparison)
#
# EXPLORATORY components:
#   N1   : FC1+FC2,  80–160 ms  — mean amp only
#   CSW  : FC1+FC2, 400–700 ms  — mean amp only (conflict slow wave)
#
# Plotting scheme (standard paired layout):
#   FC1+FC2 : N1 (purple) + N200 (blue) + CSW (teal) windows on same axes
#   Pz      : P300/P3b (red) window
#   Fz      : display only — no extraction
#
# Usage:
...
if len(sys.argv) != 2:
    print("Usage: python3 scripts/06_erp_stroop.py <participant_id>")
    sys.exit(1)
...
print("=" * 60)
print(f"ERP EXTRACTION — STROOP  |  {participant_id}")
...
plt.suptitle(
    f'{participant_id} STROOP — Butterfly plots (negative up)\n'
    f'Blue dashed = FC1, FC2 + virtual FCz · Solid = Pz · Faded = other',
    fontsize=11, y=1.01)
plt.tight_layout()
fig.savefig(str(erp_folder/f'{participant_id}_stroop_butterfly.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"  Saved: {participant_id}_stroop_butterfly.png")

# ── Step 4: ERP condition comparison plots ────────────────────────────────────
print("\nSTEP 4: ERP plots")

# 4a: FC1+FC2 — N1 + N200 + CSW
...
ax.set_title(
    f'{participant_id} STROOP — ERP at FC1+FC2 (virtual FCz)\n'
    f'Purple=N1 (80–160ms) · Blue=N200 (200–350ms) · '
    f'Teal=CSW (400–700ms) · Negative up', fontsize=10)
ticks(ax, tmin_ms, tmax_ms)
plt.tight_layout()
fig.savefig(str(erp_folder/f'{participant_id}_stroop_erp_FC1_FC2.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"  Saved: {participant_id}_stroop_erp_FC1_FC2.png")
...
ax.legend(fontsize=9, loc='upper right')
ax.set_title(
    f'{participant_id} STROOP — ERP at Pz\n'
    f'Red = P300 / P3b (300–600 ms) · Negative up', fontsize=10)
ticks(ax, tmin_ms, tmax_ms)
plt.tight_layout()
fig.savefig(str(erp_folder/f'{participant_id}_stroop_erp_Pz.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"  Saved: {participant_id}_stroop_erp_Pz.png")
...
if plotted:
    ...
    ax.legend(fontsize=9, loc='upper right')
    ax.set_title(
        f'{participant_id} STROOP — ERP at Fz (display only)\n'
        f'No component extraction · Negative up', fontsize=10)
    ticks(ax, tmin_ms, tmax_ms)
    plt.tight_layout()
    fig.savefig(str(erp_folder/f'{participant_id}_stroop_erp_Fz.png'),
                dpi=150, bbox_inches='tight')
    print(f"  Saved: {participant_id}_stroop_erp_Fz.png")
plt.close(fig)
...
fig.text(0.02, tt+0.04,
         f'{participant_id} STROOP — Topographic maps\n'
         f'120–550 ms (±25 ms avg) · Shared scale ±{gmax:.1f} µV',
         fontsize=13, va='bottom', ha='left')
fig.savefig(str(erp_folder/f'{participant_id}_stroop_topomaps_combined.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"  Saved: {participant_id}_stroop_topomaps_combined.png")
...
print("\n" + "="*60)
print(f"COMPLETE — {participant_id} STROOP  |  "
      f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("="*60)
print(f"\nNext: python3 scripts/07_group_erp_stroop.py")

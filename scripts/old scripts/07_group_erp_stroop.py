# scripts/07_group_erp_stroop.py
# ─────────────────────────────────────────────────────────────────────────────
# STEP 5 OF EEG PIPELINE: Group-level ERP analysis — STROOP
#
...
# EXPLORATORY components: N1, CSW
#
# Outputs — output/group/:
#   Waveform plots (with individual traces lw=1.2, alpha=0.45):
#     group_stroop_erp_FC1_FC2.png              side-by-side
#     group_stroop_erp_Pz.png
#     group_stroop_erp_Fz.png
#     group_stroop_erp_FC1_FC2_by_condition.png  per-condition overlay
#     group_stroop_erp_Pz_by_condition.png
#     group_stroop_erp_Fz_by_condition.png
#
#   Bar charts (p-value annotated for primary; Cohen's d for exploratory):
#     group_stroop_N200_amplitude_bars.png
#     group_stroop_N200_latency_bars.png
#     group_stroop_P300_amplitude_bars.png
#     group_stroop_P300_latency_bars.png
#     group_stroop_P3b_latency_bars.png
#     group_stroop_N1_amplitude_bars.png        (exploratory — d only)
#     group_stroop_CSW_amplitude_bars.png       (exploratory — d only, incongruent)
#
#   Effect / comparison plots:
#     group_stroop_conflict_effect.png
#     group_stroop_P3b_vs_P300_latency.png
#
#   CSVs:
#     group_stroop_summary.csv
#     group_stroop_individual_components.csv
#
# Usage:
...
print("=" * 60)
print(f"GROUP ERP ANALYSIS — STROOP")
...
# ── Component definitions (must match 06_erp_stroop.py) ──────────────────────
primary_components = {
...
for comp in all_components:
    for cond in conditions:
        for grp in groups:
            ...
            n = len(sub)
            stats_rows.append({
                'task': task, 'component': comp, 'condition': cond, 'group': grp,
                'n':             n,
...
# ── Step 3: Load epochs for grand averages ────────────────────────────────────
...
for ecfg in elec_cfgs:
    ...
    plt.suptitle(
        f"STROOP — Grand average ERPs at {ecfg['label']}\n"
        f"Green=congruent · Red=incongruent · Grey=no response\n"
        f"Dotted=control · Solid=creatine · "
        f"Bold=grand average · Faint=individual participants · Negative up",
        fontsize=13, y=1.02)
    plt.tight_layout()
    fig.savefig(str(group_folder/f"group_stroop_erp_{ecfg['fname']}.png"),
                dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved: group_stroop_erp_{ecfg['fname']}.png")

# ── Step 5: Waveform plots — by condition (control vs creatine overlaid) ──────
...
for ecfg in elec_cfgs:
    ...
    for ax, cond in zip(axes, conditions):
        for grp in groups:  # control first (dotted), creatine second (solid)
            # Individual traces
            light_col = lighten(cond_colours[cond], 0.50)
            for pid, ev in ind_evokeds.get((grp, cond), []):
                d = get_ch(ev, elec)
    ...
    plt.suptitle(
        f"STROOP — Control vs Creatine at {ecfg['label']}\n"
        f"Green=congruent · Red=incongruent · Grey=no response\n"
        f"Dotted=control · Solid=creatine · "
        f"Bold=grand average · Faint=individual participants · Negative up",
        fontsize=13, y=1.01)
    ...
    fig.savefig(str(group_folder/
                    f"group_stroop_erp_{ecfg['fname']}_by_condition.png"),
                dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved: group_stroop_erp_{ecfg['fname']}_by_condition.png")

# ── Step 6: Bar charts — amplitude and latency ────────────────────────────────
print(f"\nSTEP 6: Bar charts")
...
def make_bar_chart(comp_name, measure_col, ylabel, fname,
    ...
    plt.suptitle(
        f"STROOP — {comp_name} {ylabel_clean}: Control vs Creatine\n"
...
make_bar_chart('N1','mean_amp_uv','N1 mean amplitude (µV)',
               'group_stroop_N1_amplitude_bars.png',
               is_primary=False, invert_y=True)
# CSW — incongruent only
make_bar_chart('CSW','mean_amp_uv','CSW mean amplitude (µV)',
               'group_stroop_CSW_amplitude_bars.png',
               is_primary=False, invert_y=True,
               cond_subset=['incongruent/correct'])

# ── Step 7: Conflict effect plot ──────────────────────────────────────────────
print(f"\nSTEP 7: Conflict effect plot")
...
for ax, comp in zip(axes, effect_comps):
    ...
    ctrl_eff, creat_eff = [], []
    for gi, grp in enumerate(groups):  # control=0, creatine=1
        sub = combined[(combined['component']==comp) &
                       (combined['group']==grp)]
        for pid in sub['participant_id'].unique():
            p   = sub[sub['participant_id']==pid]
            inc = p[p['condition']=='incongruent/correct'][measure_col].values
            con = p[p['condition']=='congruent/correct'][measure_col].values
...
plt.suptitle(
    f"STROOP — Conflict effect (Incongruent − Congruent)\n"
    f"Dark grey dotted = control · Light grey solid = creatine\n"
    f"Bars=group mean · Error bars=SEM · Dots=individual participants\n"
    f"Primary Outcomes — Uncorrected statistics",
    fontsize=12, fontweight='bold', y=0.99)
plt.tight_layout(rect=[0.05, 0.08, 0.98, 0.88])
fig.savefig(str(group_folder/'group_stroop_conflict_effect.png'),
            dpi=300, bbox_inches='tight', pad_inches=0.15)
plt.close(fig)
print(f"  Saved: group_stroop_conflict_effect.png")
...
for ax, cond in zip(axes, conditions):
    for grp in groups:
        ...
        for pid in sub_p300['participant_id'].unique():
            lat_300 = sub_p300[sub_p300['participant_id']==pid]['peak_lat_ms'].values
            lat_p3b = sub_p3b[ sub_p3b[ 'participant_id']==pid]['peak_lat_ms'].values
            if len(lat_300)>0 and len(lat_p3b)>0:
                ax.scatter(lat_300[0], lat_p3b[0],
                           color=group_colours[grp], s=80, zorder=5,
                           label=group_labels[grp], alpha=0.85)
                ax.annotate(pid, (lat_300[0], lat_p3b[0]),
                            textcoords='offset points', xytext=(4,4),
                            fontsize=7, color=group_colours[grp])
...
plt.suptitle(
    f"STROOP — P3b latency vs P300 latency (300–600 ms, Pz)\n"
    f"Points on diagonal: P3b peak = P300 peak (same latency)\n"
    f"Points off diagonal: P3b and P300 peaks diverge\n"
    f"Red=control · Blue=creatine",
    fontsize=12, fontweight='bold', y=0.98)
plt.tight_layout(rect=[0, 0, 1, 0.88])
fig.savefig(str(group_folder/'group_stroop_P3b_vs_P300_latency.png'),
            dpi=300, bbox_inches='tight')
plt.close(fig)
print(f"  Saved: group_stroop_P3b_vs_P300_latency.png")

# ── Step 9: Group comparison topomaps — 5×9 grid ─────────────────────────────
# Layout: 5 columns (time points) × 9 rows
#   Rows 1–3 : Congruent    — Control / Creatine / Difference
#   Rows 4–6 : Incongruent  — Control / Creatine / Difference
#   Rows 7–9 : No response  — Control / Creatine / Difference
#
# Colour scales:
...
# ── Collect evoked data per group per condition ───────────────────────────────
def get_topo_data(evoked, t, half_win=0.025):
    """Return mean amplitude across ±half_win around t (in seconds)."""
...
# ── Build figure ──────────────────────────────────────────────────────────────
row_h   = 1.8    # inches per row
col_w   = 1.98   # inches per column
label_w = 1.62   # inches for row labels on left
cbar_h  = 0.315  # inches for colourbar strip at bottom

fig_w = label_w + col_w * n_cols + 0.5   # total width
fig_h = row_h * n_rows + cbar_h * 2 + 1.2  # total height + colourbar + title

fig = plt.figure(figsize=(fig_w, fig_h))

# Reserve space: top title + rows + two colourbar rows at bottom
title_frac  = 0.06
cbar_frac   = (cbar_h * 2) / fig_h
topo_frac   = 1.0 - title_frac - cbar_frac - 0.02

from matplotlib.gridspec import GridSpec, GridSpecFromSubplotSpec

# Main grid: n_rows topo rows
gs_main = GridSpec(
    n_rows, n_cols,
    figure=fig,
...
for cond_idx, (cond, cond_lbl, cond_col) in enumerate(topo_conditions):
    base_row = cond_idx * 3   # rows 0,3,6
...
for divider_row in [3, 6]:
    y_pos = gs_main.get_subplot_params(fig).top - \
            (divider_row / n_rows) * \
            (gs_main.get_subplot_params(fig).top -
             gs_main.get_subplot_params(fig).bottom)
    fig.add_artist(
        plt.Line2D(
            [label_w/fig_w, 0.97],
            [y_pos, y_pos],
...
# ── Colourbars at bottom ──────────────────────────────────────────────────────
cb_bottom = 0.01
cb_height = 0.025

# Amplitude colourbar (left half)
...
# Difference colourbar (right half)
...
fig.text(
    0.5, 0.995,
    f"STROOP — Group grand average topomaps: Control (N={ctrl_n}) vs "
...
topo_outpath = str(group_folder / 'group_stroop_topomaps_group_comparison.png')
fig.savefig(topo_outpath, dpi=300, bbox_inches='tight')
plt.close(fig)
print(f"  Saved: group_stroop_topomaps_group_comparison.png")
...
df_stats.to_csv(str(group_folder/'group_stroop_summary.csv'), index=False)
combined.to_csv(str(group_folder/'group_stroop_individual_components.csv'),
                index=False)
print(f"  Saved: group_stroop_summary.csv")
print(f"  Saved: group_stroop_individual_components.csv")

# ── Summary ───────────────────────────────────────────────────────────────────
print("\n" + "="*60)
print(f"COMPLETE — Group ERP STROOP")
print(f"Finished : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("="*60)
print(f"\nParticipants: {sorted(combined['participant_id'].unique())}")
print(f"\nNext: python3 scripts/08_statistics_stroop.py")

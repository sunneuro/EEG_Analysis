# EEG Analysis Pipeline — Creatine Supplementation and Cognitive Performance

**A complete EEG preprocessing and ERP analysis pipeline for Stroop and N-back tasks**

> Creatine supplementation and cognitive performance study  
...
This pipeline takes raw EEG recordings and E-Prime behavioural data from a study comparing creatine-supplemented participants against a control group on two cognitive tasks:
...
P01,John Doe,creatine,24,M
P02,Jane Smith,control,22,F
...
```

The `group` column must be exactly `creatine` or `control` (lowercase).
...
├── run_stroop_pipeline.py             ← runs full Stroop pipeline
├── run_nback_pipeline.py              ← runs full N-back pipeline
│
...
### `01_parse_eprime_nback.py` — Parse N-back Behavioural Data

**What it does:** Reads the raw E-Prime `.txt` files for the N-back task, extracts trial-level data (reaction times, accuracy, trial type, N-back level), separates practice from experimental trials, joins participant group information from `participants.csv`, and saves a clean CSV per participant.

**When to run:** Once per study, before running script 05 for N-back.

**Windows:**
```
python scripts/01_parse_eprime_nback.py
```
...
**Reads:** `data/behavioural/nback/P0X_nback.txt` + `data/participants.csv`

**Produces:** `data/behavioural/nback/P0X_nback_clean.csv` — one row per trial with participant ID, group, trial type (Target/Non-target), N-back level (ZeroList/OneList/TwoList), reaction time, accuracy, missed response flag, practice flag.

---

### `02_parse_eprime_stroop.py` — Parse Stroop Behavioural Data

**What it does:** Same as script 01 but for the Stroop task. Reads E-Prime Stroop `.txt` files, extracts congruency, reaction times, and accuracy, and saves clean CSVs.

**Windows:**
```
python scripts/02_parse_eprime_stroop.py
```
...
**Produces:** `data/behavioural/stroop/P0X_stroop_clean.csv` — one row per trial with participant ID, group, congruency (Congruent/Incongruent), reaction time, accuracy, missed response flag.

---

### `03_import_filter_eeg.py` — Import and Filter EEG
...
python scripts/03_import_filter_eeg.py P01 nback  JD_Nback_22_05_2026.vhdr
```

**Mac:**
```
python3 scripts/03_import_filter_eeg.py P01 stroop JD_Stroop_22_05_2026.vhdr
python3 scripts/03_import_filter_eeg.py P01 nback  JD_Nback_22_05_2026.vhdr
```
...
### `04_ica.py` — ICA Artefact Removal
...
**Windows:**
```
python scripts/04_ica.py P01 stroop
python scripts/04_ica.py P01 nback
```
...
**Windows:**
```
python scripts/05_epochs.py P01 stroop
python scripts/05_epochs.py P01 nback
```

**Mac:**
```
python3 scripts/05_epochs.py P01 stroop
python3 scripts/05_epochs.py P01 nback
```
...
### `06_erp_stroop.py` — Stroop ERP Extraction (per participant)
...
**Windows:**
```
python scripts/06_erp_stroop.py P01
```

**Mac:**
```
python3 scripts/06_erp_stroop.py P01
```
...
### `06_erp_nback.py` — N-back ERP Extraction (per participant)

**What it does:** Same as `06_erp_stroop.py` but for the N-back task with task-appropriate components.

**Windows:**
```
python scripts/06_erp_nback.py P01
```
...
### `07_group_erp_stroop.py` — Stroop Group ERP Analysis

**What it does:** Loads all individual ERP component CSVs, computes group grand averages, generates group-level waveform plots with individual participant traces behind the group average, produces bar charts comparing control and creatine groups with p-value annotations.

**Windows:**
```
python scripts/07_group_erp_stroop.py
```

**Mac:**
```
python3 scripts/07_group_erp_stroop.py
```
...
### `07_group_erp_nback.py` — N-back Group ERP Analysis

**Windows:**
```
python scripts/07_group_erp_nback.py
```
...
### `08_statistics_stroop.py` — Stroop Statistical Analysis
...
**Windows:**
```
python scripts/08_statistics_stroop.py
```
...
### `08_statistics_nback.py` — N-back Statistical Analysis

**Windows:**
```
python scripts/08_statistics_nback.py
```
...
cd /Users/SK/eeg_analysis
source .venv/bin/activate
python3 run_stroop_pipeline.py
python3 run_nback_pipeline.py
```
...
**Windows:**
```
python run_stroop_pipeline.py P01
python run_nback_pipeline.py P01 P02 P04
```

**Mac:**
```
python3 run_stroop_pipeline.py P01
python3 run_nback_pipeline.py P01 P02 P04
```
...
# Step 1 — behavioural data (run once)
  ...
  Windows:  python  scripts/01_parse_eprime_nback.py
  Mac:      python3 scripts/01_parse_eprime_nback.py

  Windows:  python  scripts/02_parse_eprime_stroop.py
  Mac:      python3 scripts/02_parse_eprime_stroop.py

# Step 2 — EEG import and filter (per participant per task)

  Windows:  python  scripts/03_import_filter_eeg.py P01 stroop JD_Stroop_22_05_2026.vhdr
  Mac:      python3 scripts/03_import_filter_eeg.py P01 stroop JD_Stroop_22_05_2026.vhdr

  Windows:  python  scripts/03_import_filter_eeg.py P01 nback  JD_Nback_22_05_2026.vhdr
  Mac:      python3 scripts/03_import_filter_eeg.py P01 nback  JD_Nback_22_05_2026.vhdr

# Step 3 — ICA (per participant per task)
# → Review ICA plots in output/erp/ before proceeding

  Windows:  python  scripts/04_ica.py P01 stroop
  Mac:      python3 scripts/04_ica.py P01 stroop

# Step 4 — Epoching (per participant per task)

  Windows:  python  scripts/05_epochs.py P01 stroop
  Mac:      python3 scripts/05_epochs.py P01 stroop

# Step 5 — Individual ERP (per participant, run both tasks)

  Windows:  python  scripts/06_erp_stroop.py P01
  Mac:      python3 scripts/06_erp_stroop.py P01

  Windows:  python  scripts/06_erp_nback.py P01
  Mac:      python3 scripts/06_erp_nback.py P01

# Step 6 — Group ERP (once, after all participants complete)

  Windows:  python  scripts/07_group_erp_stroop.py
  Mac:      python3 scripts/07_group_erp_stroop.py

  Windows:  python  scripts/07_group_erp_nback.py
  Mac:      python3 scripts/07_group_erp_nback.py

# Step 7 — Statistics (once)

  Windows:  python  scripts/08_statistics_stroop.py
  Mac:      python3 scripts/08_statistics_stroop.py

  Windows:  python  scripts/08_statistics_nback.py
  Mac:      python3 scripts/08_statistics_nback.py
```
...
| File | What it shows |
|---|---|
| `group_stroop_erp_FC1_FC2.png` | Grand average waveforms — control (left panel), creatine (right panel). Bold line = group average. Thin semi-transparent lines = individual participants. |
| `group_stroop_erp_FC1_FC2_by_condition.png` | One panel per condition (congruent, incongruent, no response). Both groups overlaid: solid = creatine, dashed = control. |
| `group_stroop_N200_amplitude_bars.png` | Bar chart: control (left bar) vs creatine (right bar) per condition. Error bars = SEM. Dots = individual participants. Significance bracket with p-value stars above each pair. |
...
| `group_stroop_individual_components.csv` | All individual participant values — used by script 08 for statistics. |
...
**Cognitive meaning:** Sustained conflict resolution — prolonged frontal engagement during resolution of incongruent Stroop trials. Visible in the creatine group at the grand average level.
...
**Fix:** Re-run script 03. Make sure the correct 32-channel recording file is being used.
...
### Participants
- 8 participants: 4 creatine (P01–P04), 4 control (P05–P08)
- Between-subjects design

### EEG recording
- System: BrainVision actiCHamp, 32 channels, 500 Hz
- Online reference: Cz (re-added in script 03)
...
*EEG Analysis Pipeline | Creatine supplementation and cognitive performance study*

...
#
# Cross-platform pipeline runner for the Stroop ERP analysis.
...
#   Step 3: 08_statistics_stroop.py — statistical comparisons
#
# All participants included (P01–P08).
# Outlier flags recorded in output CSV — no participants excluded.
#
# Usage:
#   python run_stroop_pipeline.py              # all participants
#   python run_stroop_pipeline.py P01         # single participant
#   python run_stroop_pipeline.py P01 P02 P04 # specific participants
#
# Requirements:
...
# Windows: enable via os.system('') call below
import os
...
# P03 — high miss rate / reversed amplitude (included, flagged in output)
# P05 — low congruent trials n=25            (included, flagged)
# P08 — low congruent trials n=6             (included, flagged)
...
print(f"\n{BOLD}============================================================{NC}")
print(f"{BOLD} STROOP ERP PIPELINE — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{NC}")
print(f"{BOLD}============================================================{NC}")
...
print(f"{BOLD}STEP 1: Individual ERP extraction (06_erp_stroop.py){NC}")
print("------------------------------------------------------------")

for pid in PARTICIPANTS:

    epo_file = project_root / 'data' / 'processed' / f'{pid}_stroop_epo.fif'
    ...
    print(f"\n  {BLUE}▶ {pid}{NC}")
    success, elapsed = run_script('06_erp_stroop.py', args=[pid], label=pid)
...
print(f"{BOLD}STEP 2: Group ERP analysis (07_group_erp_stroop.py){NC}")
print("------------------------------------------------------------")

success, elapsed = run_script('07_group_erp_stroop.py')
...
# ── STEP 3: Statistics ────────────────────────────────────────────────────────
print(f"{BOLD}STEP 3: Statistical analysis (08_statistics_stroop.py){NC}")
print("------------------------------------------------------------")

success, elapsed = run_script('08_statistics_stroop.py')
...
print(f"{BOLD}============================================================{NC}")
print(f"{BOLD} STROOP PIPELINE COMPLETE — {mins}m {secs}s{NC}")
...
print()
print(f"  Individual ERPs : output/erp/P0X_stroop_*.png")
print(f"                    output/erp/P0X_stroop_erp_components.csv")
print(f"  Group plots     : output/group/group_stroop_*.png")
print(f"  Group CSVs      : output/group/group_stroop_*.csv")
print(f"  Statistics      : output/stats/stroop_statistics_primary.csv")
print(f"                    output/stats/stroop_statistics_exploratory.csv")
print(f"                    output/stats/stroop_statistics_sensitivity.csv")
print()
print(f"  Outlier notes (all included, flagged in CSV):")
print(f"    P03 — high miss rate / reversed amplitude")
print(f"    P05 — low congruent trials n=25")
print(f"    P08 — low congruent trials n=6")
print()

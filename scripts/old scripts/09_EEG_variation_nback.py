# scripts/09_EEG_variation_nback.py
# ─────────────────────────────────────────────────────────────────────────────
# BETWEEN-SUBJECT VARIABILITY ANALYSIS — N-BACK TASK
#
# This script loads the N-back group ERP component data and compares
# between-subject variability (variance/standard deviation) between
# the Control and Creatine groups.
#
# Outputs:
#   CSV files in output/stats/:
#     nback_variability_descriptives.csv   - Descriptive statistics
#     nback_variability_statistics.csv     - Inferential statistics (Levene's test)
#
#   Figures in output/group/:
#     group_nback_variability_nontarget.png
#     group_nback_variability_target_hit.png
#     group_nback_variability_target_miss.png
#
# Usage:
#   python3 scripts/09_EEG_variation_nback.py
# ─────────────────────────────────────────────────────────────────────────────
...
print("=" * 70)
print(f"N-BACK EEG VARIABILITY ANALYSIS")
print(f"Started : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print("=" * 70)

# ── Load Data ─────────────────────────────────────────────────────────────────
csv_path = group_folder / 'group_nback_individual_components.csv'
if not csv_path.exists():
    print(f"ERROR: {csv_path} not found. Run 07_group_erp_nback.py first.")
    sys.exit(1)
...
descriptives_path = stats_folder / 'nback_EEG_variability_descriptives.csv'
statistics_path = stats_folder / 'nback_EEG_variability_statistics.csv'
...
for cond_code in conditions:
    ...
    # Save figure
    fname = f"group_nback_variability_{file_suffixes[cond_code]}.png"
    plt.suptitle(
        f"N-BACK {cond_lbl.upper()} EEG VARIABILITY — Control vs Creatine",

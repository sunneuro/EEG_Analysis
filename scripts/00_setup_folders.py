...
def get_participants_csv_template(group1, group2):
    return f"""participant_id,name,group,age,sex
P01,Participant One,{group1},24,M
P02,Participant Two,{group2},22,F
P03,Participant Three,{group1},23,F
P04,Participant Four,{group1},25,M
P05,Participant Five,{group2},24,F
P06,Participant Six,{group2},23,M
P07,Participant Seven,{group2},22,F
P08,Participant Eight,{group1},24,M
"""
...
FOLDER_INSTRUCTIONS = {
    ...
    'data/participants.csv': {
        ...
        'instructions': [
            ...
            '  group           — exact group name (e.g., control, treatment)',
            '  age             — age in years',
            '  sex             — M or F',
            '',
...
def mode_create():
    ...
    rule()
    print("This pipeline compares two experimental groups.")
    group1 = ask("Enter name for Group 1 (e.g., 'control', 'placebo', 'A')", default='control').lower()
    group2 = ask("Enter name for Group 2 (e.g., 'creatine', 'drug', 'B')", default='treatment').lower()
    ...
    if not csv_path.exists():
        try:
            csv_path.write_text(get_participants_csv_template(group1, group2), encoding='utf-8')
    ...
    if csv_path.exists():
        ...
        try:
            ...
            with open(csv_path, encoding='utf-8') as f:
                ...
                detected_groups = list(set([r.get('group', '?') for r in rows if r.get('group')]))
                if len(detected_groups) != 2:
                    cross(f"Expected exactly 2 groups, but found {len(detected_groups)}: {detected_groups}")
                    issues.append(f"Expected 2 groups in participants.csv, found {len(detected_groups)}")
                else:
                    g1, g2 = sorted(detected_groups)
                    tick(f"Detected 2 groups: {g1}, {g2}")
                for row in rows:
                    grp = row.get('group','?')
                    grp_col = GREEN if grp==detected_groups[0] else BLUE if len(detected_groups)>1 and grp==detected_groups[1] else YELLOW
